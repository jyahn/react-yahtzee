...
class FullHouse extends Rule {
  ...
  evalRoll(dice) {
    let counter = {};
    for (let die of dice) {
      counter[die] = counter[die] + 1 || 1
    }
    if (Object.values(counter).length === 2 && Object.values(counter).includes(3)) {
      return this.score
    } else {
      return 0
    }
...
class SmallStraight extends Rule {
  ...
  evalRoll(dice) {
    ...
    return d.size === 4 && ((!d.has(1) && !d.has(2)) || (!d.has(5) && !d.has(6)) || (!d.has(1) && !d.has(6))) ? this.score : 0;
  }
}
